...
void handleActive1() {
  ...
  if(STEPS_taken != STEPS){
    if(currentMode == mode_normal){
      ...
      frequencySweepRaw();
      if (!anomalyDetection()) { // if no anomaly detected
        highResCounter++;
        if(highResCounter >= 3) { // need 3 consecutive normal readings
          currentMode = mode_normal;
          highResCounter = 0; // reset
        }
      } else { // if anomaly detected
        highResCounter = 0; // restart counter
      }
    } 
  }
  if (STEPS_taken >= STEPS) {
